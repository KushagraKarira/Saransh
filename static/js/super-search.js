...
(function () {
   ...
   function xmlToJson(xml) {
      ...
      var textNodes = [].slice.call(xml.childNodes)
         .filter(function (node) {
            return node.nodeType === 3;
         });
      if (xml.hasChildNodes() && xml.childNodes.length === textNodes.length) {
         obj = [].slice.call(xml.childNodes)
   ...
   xmlhttp.onreadystatechange = function () {
      ...
      var node = (new DOMParser)
         .parseFromString(xmlhttp.responseText, 'text/xml');
   ...
   searchInputEl.addEventListener('input', function onInputChange() {
      ...
      if (posts.title === undefined) {
         matchingPosts = posts.filter(function (post) {
            if ((post.title + '')
               .toLowerCase()
               .indexOf(currentInputValue) !== -1 || (post.description + '')
               .toLowerCase()
